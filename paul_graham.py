...
chroma_collection = chroma_db.get_or_create_collection(
    "paul_graham", embedding_function=embedding_fn
)
...
def get_query_results(query_text: str, n_results: int = 5) -> str:
    ...
    if (
        res.get("metadatas")
        and isinstance(res["metadatas"], list)
        and len(res["metadatas"]) > 0
    ):
        keywords = [
            item.get("keyword", "") if isinstance(item, dict) else ""
            for item in res["metadatas"][0]
        ]
    else:
        keywords = [""] * len(docs)  # Assign empty keywords if metadata is missing

    return "; ".join(
        [f"{keyword}: {information}" for keyword, information in zip(keywords, docs)]
    )
